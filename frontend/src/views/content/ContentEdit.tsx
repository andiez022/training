...
const BoardCreate = () => {
  ...
  const { id } = useParams();
  const { data: dataItem } = useQuery(['annItem', id], () => api.data.fetchDataById('content', id || ''));

  // ? edit data
  const editData = useMutation((values: any) => api.data.editData('content', values), {
    onSuccess: () => {
      navigate('/content');
    ...
    <div>
      ...
      <div className="ann-create-container">
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <div className="ann-create-container__input-video">
            <label htmlFor="video">제목</label>
            <input type="text" placeholder="제목을 입력해주세요." name="video" value={formik.values.video} onChange={formik.handleChange} />
          ...
          <div className="ann-create-container__input__button">
            ...
            <button type="button" className="ann-create-container__input__button-cancel" onClick={() => navigate('/content')}>
              취소
            </button>
          </div>
        </form>
